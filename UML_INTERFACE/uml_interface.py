...
Description:
    ...
    attributes, and relationships. It allows users to add, delete, rename 
    classes, attributes, and manage relationships between classes.
"""
...
class UMLInterface:
    def __init__(self):
        ...
        self.relationship_manager = RelationshipManager(self.manager.class_manager)  # Initialize RelationshipManager
        
        # Pass class_manager, attribute_manager, and relationship_manager to UMLListing
        self.uml_listing = UMLListing(self.manager.class_manager, self.manager.attribute_manager, self.relationship_manager)
    ...
    def load_data(self, filename=None):
        ...
        if uml_data:
            for cls in uml_data.get('classes', []):
                self.manager.class_manager.add_class(cls["name"])  # Load classes
                for attr in cls.get("attributes", []):
                    self.manager.attribute_manager.add_attr(cls["name"], attr["name"])  # Load attributes
    ...
    def run(self):
        while True:
            ...
            if main_command == "help":
                ...
                self.uml_listing.list_classes()  # List classes, attributes, and relationships

            # Relationship management commands
            elif main_command.startswith("add_rel") and len(parts) == 4:
                class_name_a = parts[1]
                class_name_b = parts[2]
                relationship_type = parts[3]
                self.relationship_manager.add_relationship(class_name_a, class_name_b, relationship_type)
                
            elif main_command.startswith("delete_rel") and len(parts) == 4:
                class_name_a = parts[1]
                class_name_b = parts[2]
                relationship_type = parts[3]
                self.relationship_manager.delete_relationship(class_name_a, class_name_b, relationship_type)
